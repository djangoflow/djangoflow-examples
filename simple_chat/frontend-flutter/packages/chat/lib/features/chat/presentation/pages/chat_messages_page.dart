...
class ChatMessagesListPage extends StatefulWidget with AutoRouteWrapper {
  ...
  Widget wrappedRoute(BuildContext context) {
    ...
    final websocketUri = AppDomain.getChatWebsocketUri(
      environment: env,
      roomId: roomId,
      token: token,
    );
    ...
    return MultiBlocProvider(
      key: ValueKey(roomId),
      providers: [
        BlocProvider<DjangoflowWebsocketCubit>(
          create: (context) {
            final djangoflowWebsocketCubit = DjangoflowWebsocketCubit();
            if (websocketUri != null) {
              djangoflowWebsocketCubit.connectToUri(websocketUri);
            }

            return djangoflowWebsocketCubit;
          },
        ),
...
class _ChatMessagesListPageState extends State<ChatMessagesListPage>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      ...
      if (websocketUri != null) {
        context.read<DjangoflowWebsocketCubit>().connectToUri(websocketUri);
      }
    } else {
      context.read<DjangoflowWebsocketCubit>().disconnect();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final websocketConnectionStateMessage = context.select(
      (DjangoflowWebsocketCubit c) => c.state.connectionStateMessage,
    );
    return DjangoflowWebsocketBlocListener(
      ...
      listener: (context, state) {
        ...
        if (data != null) {
          for (final u in data[usersKey] ?? []) {
            final user = RoomUser.fromJson(u);
            chatCubit.addRoomUser(user);
          }
          for (final m in data[messagesKey] ?? []) {
