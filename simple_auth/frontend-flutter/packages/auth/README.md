...
- [ ] Generate api_repository,freezed, json_serializable, auto_route etc files `cd packages/auth && flutter pub run build_runner build --delete-conflicting-outputs` (you must regenerate this whenever openapi changes)
- [ ] Follow TODO comments for [auth_cubit.dart](packages/auth/lib/features/authentication/blocs/auth_cubit.dart)
- [ ] Configure firebase using `flutterfire configure` in `packages/auth` directory. More info: https://pub.dev/packages/flutterfire_cli
      - In order to enable fully enable Google sign in for all platforms, follow these steps - [ ] You don't need to include the google-services.json file in your app unless you
      ...
      - [ ] For Android, make sure you have added SHA-1 key in firebase console.
      - [ ] For iOS, go to packages/auth/ios/Runner/Info.plist and replace `CFBundleURLSchemes` with `GoogleService-Info.plist` key `REVERSED_CLIENT_ID`
        - If you face `PlatformException(missing-config, GoogleService-Info.plist file not found and clientId was not provided programmatically., null, null)` then please check if you have added `GoogleService-Info.plist` file in `packages/auth/ios/Runner` directory. If not, please add it and try again. Follwo this if helps https://www.kindacode.com/article/flutter-correctly-adding-googleservices-info-plist-to-ios/
      - [ ] For web, go to [google_sign_in_web](https://pub.dev/packages/google_sign_in_web) and update the `packages/auth/web/index.html` file's meta tag with name `google-signin-client_id`

## Other TODO

- [ ] Update fastlane .env files in `packages/auth/android/fastlane` and `packages/auth/ios/fastlane` following the `sample.env.txt` file.
- [ ] Grant repository rights in github so that firebase setup works.
- [ ] Update firebase deployment target and setup. To do so, run `firebase init hosting`. After completing the steps a `firebase.json` will be generated. A generated `example_firebase.json` file will be generated already. More information: https://firebase.google.com/docs/hosting/quickstart After setup, please modify the `firebase.json` file with the example one(modify `target`s mostly). For `target` names, dev webapp should have `djangoflow-examples-dev` and production `djangoflow-examples-prod` as target names. And site-id is the first part of the domain. For example: `example.web.app` here `example` is the site-id. Follow this link for more information: https://firebase.google.com/docs/cli/targets#set-up-deploy-target-hosting After that a `.firebaserc` file will be generated with the target information. After that you can deploy and see if deployment works for both prod and dev. If you see "HTTP Error: 404, Requested entity was not found." that means the site-id associated with the target doesn't exist. In that case, new site can be generated by `firebase hosting:sites:create your-site-id` command. After that try `firebase deploy --only hosting:your-target-name` then it should deploy to the target.
- [ ] Update github workflows file, see [web deployment](#web-app-deployment)
- [ ] Update Smart App Banner for web app after app was deployed on appstore. In `packages/auth/web/index.html` find line with `<meta name="apple-itunes-app" content="app-id=appStoreID">`, here uncomment it and replace `appStoreID` with actual app store id for the app.
...
The way this project is organised inside the `packages/auth/lib` folder is as follows:
...
- After selecting a distribution certificate for signing the build, Go to `auth/ios/fastlane/` directory.
...
After configuring this, go to `/auth/ios` folder and run `fastlane beta` and `fastlane` make ask for authentication for appstore connect and from then it will take care of the rest.
...
- Now go to `./web/.well-known` directory. Open `apple-app-site-association` file and replace all the `{team_id}` with your apple development team id. For example if, `team_id = your_team_id` and `bundle_id = com.djangoflow.auth` then `"apps": ["{team_id}.com.djangoflow.auth"]` should become `"apps": ["your_team_id.com.djangoflow.auth"]` After deploying the web app the process is done.

Now you've enabled Deep linking for iOS in your application. Test it by running for example this auth project `xcrun simctl openurl booted https://your.domain.com` should open the app in the running simulator if installed.
